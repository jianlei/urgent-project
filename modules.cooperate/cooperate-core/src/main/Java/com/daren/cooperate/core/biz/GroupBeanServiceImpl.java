...
public class GroupBeanServiceImpl extends GenericBizServiceImpl implements IGroupBeanService {
    ...
    public Map createGroup(@FormParam("group_name")String group_name, @FormParam("group_logo")String group_logo,
                           @FormParam("ids")String ids,@FormParam("req_msg")String req_msg) {
        Map map = new HashMap();
        int result = -1;
        try{
            ...
            //groupBasicBean.setCreate_userid();
    ...
    public Map getGroupList(@PathParam("page")Integer page, @PathParam("page_size")Integer page_size) {
        Map map = new HashMap();
        int result = -1;
        try{
            long user_id = 1;
    ...
    @Produces("application/json;charset=utf-8")
    @Path("/replyReq")
    public Map replyReq(@FormParam("group_id")Integer group_id, @FormParam("res_type")Integer res_type) {
        Map map = new HashMap();
        int result = -1;
        try{
            if(group_id!=null && res_type!=null){
                Long user_id = 1l;
