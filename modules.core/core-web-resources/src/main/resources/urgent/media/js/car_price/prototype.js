...
if (Prototype.Browser.Opera) {
    ...
    Element.Methods.setOpacity = function(element, value) {
        element = $(element);
        element.style.opacity = (value == 1 || value === '') ? '' : (value < 0.00001) ? 0 : value;
        if (value == 1) if (element.tagName == 'IMG' && element.width) {
            element.width++;
            element.width--
        } else try {
...
Object.extend(Selector, {
    ...
    handlers: {
        ...
        unique: function(nodes) {
            ...
            var results = [],
                n;
            for (var i = 0, l = nodes.length; i < l; i++) if (!(n = nodes[i])._countedByPrototype) {
...
Form.Element.Serializers = {
    ...
    select: function(element, index) {
        if (Object.isUndefined(index)) return this[element.type == 'select-one' ? 'selectOne' : 'selectMany'](element);
        else {
...
Object.extend(Event, (function() {
    ...
    function destroyCache() {
        for (var id in cache) for (var eventName in cache[id]) cache[id][eventName] = null
    }
